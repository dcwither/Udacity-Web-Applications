...
class RegisterHandler(UserHandler):
    def get(self):
        self.render_html("signup-form.html")
    ...
    def post(self):
        ...
        if have_error:
            self.render_html('signup-form.html', **params)
        else:
...
class LoginHandler(UserHandler):
    def get(self):
        self.render_html('login-form.html')
        return
    def post(self):
        username = self.request.get('username')
        password = self.request.get('password')
        ...
        if user:
            ...
            self.render_html('login-form.html', error = "Invalid Login")
        return
